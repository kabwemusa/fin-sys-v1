...
<div class="p-6 lg:p-8 max-w-5xl mx-auto">
    ...
    @if($applications->isEmpty())
        {{-- Empty state --}}
        <div class="portal-panel rounded-3xl p-12 text-center">
            <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-[#FEF9E1] mb-4">
            ...
            <a href="{{ route('home') }}#products"
               class="portal-action inline-flex items-center gap-2 rounded-xl bg-[#E98C00] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#C97A00]">
                Apply for a Loan
            </a>
        </div>
    @else
        ...
        <div class="portal-panel rounded-3xl overflow-hidden">
            {{-- Header --}}
            <div class="hidden md:grid grid-cols-12 gap-4 px-6 py-3 bg-[#FEF9E1] border-b border-gray-100 text-xs font-medium text-gray-400 uppercase tracking-wider">
            ...
            <div class="divide-y divide-gray-50">
                @foreach($applications as $app)
                    ...
                    <a href="{{ route('portal.loan.detail', $app->reference) }}"
                       class="portal-card-hover grid grid-cols-12 items-center gap-4 px-6 py-4 transition-colors hover:bg-[#FEF9E1]/60 group">
                        {{-- Reference --}}
                        <div class="col-span-12 md:col-span-3">
                            <p class="text-sm font-medium text-[#E98C00] group-hover:underline tracking-wide">{{ $app->reference }}</p>
        ...
        <div class="mt-4 text-center">
            <a href="{{ route('home') }}#products" class="portal-action text-sm font-medium text-[#E98C00] hover:underline">

                ...
                <a href="{{ route('admin.customer.detail', $customer->id) }}"
                   class="grid grid-cols-12 gap-3 px-6 py-4 hover:bg-gray-50/60 transition-colors items-center group">
                    <div class="col-span-12 md:col-span-4 flex items-center gap-3">
                        <div class="w-8 h-8 rounded-full bg-[#1B4F72]/10 flex items-center justify-center shrink-0">
                            <span class="text-[#1B4F72] text-xs font-semibold">{{ strtoupper(substr($customer->user->name, 0, 1)) }}</span>
                        </div>
                        <div>
                            <p class="text-sm font-medium text-gray-800 group-hover:text-[#1B4F72] transition-colors">{{ $customer->user->name }}</p>

<div class="p-6 lg:p-8 max-w-6xl mx-auto space-y-5">
    ...
    <div class="flex items-center justify-between">
        ...
        <button wire:click="openCreate"
            class="inline-flex items-center gap-2 px-5 py-2.5 bg-[#1B4F72] text-white text-sm font-medium rounded-xl hover:bg-[#154060] transition-colors shadow-sm">
            + New Product
        </button>
    </div>
        ...
        @foreach($products as $product)
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-50 flex items-start justify-between">
                    ...
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $product->is_active ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-400' }}">
                        {{ $product->is_active ? 'Active' : 'Inactive' }}
                    </span>
                </div>
                <div class="px-5 py-4 space-y-2 text-sm">
                    <div class="flex justify-between"><span class="text-gray-400">Rate</span><span class="font-semibold text-[#1B4F72]">{{ $product->interest_rate }}%/mo</span></div>
                ...
                <div class="px-5 pb-4 flex gap-2">
                    <button wire:click="openEdit({{ $product->id }})"
                        class="flex-1 py-2 text-xs font-medium text-[#1B4F72] bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors">
                        Edit
                    </button>
                    <button wire:click="toggleActive({{ $product->id }})"
                        class="flex-1 py-2 text-xs font-medium {{ $product->is_active ? 'text-gray-500 bg-gray-100 hover:bg-gray-200' : 'text-emerald-600 bg-emerald-50 hover:bg-emerald-100' }} rounded-lg transition-colors">
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-end">
            ...
            <div class="relative bg-white h-full w-full max-w-lg shadow-2xl overflow-y-auto">
                <div class="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between z-10">
                    ...
                    <button wire:click="$set('showForm', false)" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors">
                ...
                <form wire:submit.prevent="save" class="px-6 py-6 space-y-4">
                    ...
                    <div>
                        ...
                        <select wire:model="type" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#1B4F72]">
                    ...
                    <div>
                        ...
                        <textarea wire:model="description" rows="2" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#1B4F72] resize-none"></textarea>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1">Min Amount (ZMW) *</label>
                            <input wire:model="min_amount" type="number" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#1B4F72]">
                        ...
                        <div>
                            ...
                            <input wire:model="max_amount" type="number" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#1B4F72]">
                        ...
                        <div>
                            ...
                            <input wire:model="min_tenure" type="number" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#1B4F72]">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1">Max Tenure (months) *</label>
                            <input wire:model="max_tenure" type="number" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#1B4F72]">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Interest Rate (%/month) *</label>
                        <input wire:model="interest_rate" type="number" step="0.01" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#1B4F72]">
                    ...
                    <div class="flex gap-3 pt-4 border-t border-gray-100">
                        <button type="button" wire:click="$set('showForm', false)"
                            class="flex-1 py-2.5 rounded-xl text-sm font-medium text-gray-500 border border-gray-200 hover:bg-gray-50 transition-colors">
                            Cancel
                        </button>
                        <button type="submit"
                            class="flex-1 py-2.5 rounded-xl text-sm font-medium bg-[#1B4F72] text-white hover:bg-[#154060] transition-colors">

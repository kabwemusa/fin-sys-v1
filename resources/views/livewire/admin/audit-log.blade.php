    ...
    <div class="flex flex-col sm:flex-row gap-3">
        <div class="relative flex-1">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search actions…"
                class="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#1B4F72] transition-all">
        </div>
                ...
                <div class="px-6 py-4">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-start gap-3">
                            <div class="w-8 h-8 rounded-full bg-[#1B4F72]/10 flex items-center justify-center shrink-0 mt-0.5">
                                <span class="text-[#1B4F72] text-xs font-semibold">{{ strtoupper(substr($activity->causer?->name ?? 'S', 0, 1)) }}</span>
                            </div>
                            <div>
                                <p class="text-sm text-gray-700">
                                    ...
                                    @if($activity->subject_type)
                                        ...
                                        <span class="font-medium text-[#1B4F72]">{{ class_basename($activity->subject_type) }} #{{ $activity->subject_id }}</span>
                                    @endif
                                </p>
